...
# February 2019
# This script tests the color function of filterizePy package.
...
def check_output_type(input_img, color(input_img)):
    """
    This function checks the output image format is one of the valid image formats,
    and if it matches the input image format.
    """
...
def check_output_dimension(input_img, color(input_img)):
    """
    ...
    output_img= skimage.io.imread(color(input_img))
    in_width, in_height,in_rgb = input_img.shape
    out_width, out_height,out_rgb = output_img.shape
    assert in_width == out_width and in_height==out_height, "The output dimension is not the same as the input dimension."
